#!/usr/bin/env bash
# data path is /home/mysql/data
# programe path is /usr/local/mysql
version=5.7.21
boostVersion=1_59_0
...
cur_dir=`pwd`
cd $cur_dir
boostDir=$cur_dir/boost_$boostVersion
tar zxvf mysql-$version.tar.gz
yum install -y cmake
cd $cur_dir/mysql-$version/
cmake -DCMAKE_INSTALL_PREFIX=/usr/local/mysql/ -DDOWNLOAD_BOOST=1 -DWITH_BOOST=$boostDir -DDEFAULT_CHARSET=utf8mb4 -DDEFAULT_COLLATION=utf8mb4_general_ci -DEXTRA_CHARSETS=all -DENABLED_PROFILING=ON -DWITH_READLINE=1 -DWITH_DEBUG=0 -DWITH_INNOBASE_STORAGE_ENGINE=1 -DMYSQL_DATADIR=/home/mysql/data/ -DMYSQL_UNIX_ADDR=/tmp/mysql.sock -DMYSQL_TCP_PORT=3306 -DENABLED_LOCAL_INFILE=1
make && make install
cd ../
